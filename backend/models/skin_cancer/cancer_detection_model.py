    ...
    def predict(self, img_path):
        ...
        with torch.no_grad():
            ...
            if cancer_prob > non_cancer_prob:
                if cancer_prob > 60:
                    return f"Skin Cancer Detected (Confidence: {cancer_prob:.1f}%)"
                else:
                    return f"Possible Skin Cancer (Low Confidence: {cancer_prob:.1f}%)\nRecommend further medical examination"
            else:
                return f"No Skin Cancer Detected (Confidence: {non_cancer_prob:.1f}%)"
